...
// Configurações específicas de Educação Física
$titulo = "Educação Física Digital";
$subtitulo = "Biomecânica, Performance e Esportes Imersivos";
$icone = "fitness_center";
$cor = "red"; // Cor Vermelha para Ed. Física
$videoID = "vW6pX_n_68o"; // Vídeo VR de Esportes Radicais / Performance
$videoTitulo = "Biomecânica em Movimento: Esportes 360°";
$videoTempo = "05:20";

$extra_css = "
<style>
    .video-card { background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(255, 255, 255, 0.05); transition: all 0.3s ease; }
    .video-card:hover { border-color: #ef4444; transform: translateY(-8px); background: rgba(239, 68, 68, 0.05); }
    .play-overlay { background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(2px); opacity: 0; transition: opacity 0.3s ease; }
    .video-card:hover .play-overlay { opacity: 1; }
    .qr-box { background: #fff; padding: 16px; border-radius: 2rem; display: inline-block; box-shadow: 0 0 40px rgba(239, 68, 68, 0.2); }
</style>";
    ...
    <div class='max-w-4xl mx-auto space-y-12'>
        ...
        <header class='flex justify-between items-center'>
            <div class='flex items-center gap-6'>
                <div class='w-20 h-20 bg-red-500/10 rounded-[2rem] flex items-center justify-center border border-red-500/20 shadow-[0_0_20px_rgba(239,68,68,0.1)]'>
                    <span class='material-symbols-outlined text-red-400 text-5xl'>{$icone}</span>
                </div>
                <div>
                    <h1 class='text-4xl font-bold headline text-white'>{$titulo}</h1>
                    <p class='text-red-500/60 font-bold uppercase tracking-widest text-[10px] mt-1'>{$subtitulo}</p>
                </div>
            </div>
        ...
        <section class='space-y-6 text-center'>
             <h2 class='text-xl font-bold text-white flex items-center justify-center gap-2 headline'>
                <span class='material-symbols-outlined text-red-400'>sports_score</span>
                Análise de Desempenho VR
            </h2>
                ...
                <div class='relative h-64 bg-slate-900'>
                    ...
                    <div class='play-overlay absolute inset-0 flex items-center justify-center'>
                        <div class='w-20 h-20 bg-red-500 rounded-full flex items-center justify-center shadow-lg'>
                            <span class='material-symbols-outlined text-slate-950 text-5xl'>directions_run</span>
                        </div>
                    </div>
                </div>
                <div class='p-8'>
                    <h3 class='text-2xl font-bold text-white mb-2'>{$videoTitulo}</h3>
                    <p class='text-slate-500 text-xs uppercase font-bold tracking-widest'>Treinamento Imersivo • VR 360° • {$videoTempo}</p>
                </div>
            </div>
        </section>

        <section class='glass p-12 rounded-[4rem] border-red-500/10 text-center bg-red-500/[0.01]'>
            <h2 class='text-2xl font-bold text-white headline mb-4'>Sincronizar Monitoramento</h2>
            <p class='text-slate-400 text-sm mb-8'>Escanear para visualizar a fisiologia do exercício e análise de movimento em tempo real no headset.</p>
            <div class='qr-box'>
                <img src='https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=https://youtu.be/{$videoID}' class='w-40 h-40'>
            </div>
            <p class='mt-6 text-red-400 font-mono text-[10px] tracking-widest uppercase'>Protocolo: PHYS-TRAINING-VR</p>
        </section>
    </div>
</div>
