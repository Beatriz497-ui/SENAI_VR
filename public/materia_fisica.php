...
// Configurações específicas da Física
$titulo = "Física & Mecânica";
$subtitulo = "Simulação de Leis e Forças Cinéticas";
$icone = "precision_manufacturing";
$cor = "orange"; // Cor laranja para Física (energia/mecânica)
$videoID = "H9f796U_F8w"; // Vídeo sobre Leis da Física e Gravidade em VR
$videoTitulo = "Leis de Newton e Força G em 360°";
$videoTempo = "05:40";

$extra_css = "
<style>
    .video-card { background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(255, 255, 255, 0.05); transition: all 0.3s ease; }
    .video-card:hover { border-color: #f97316; transform: translateY(-8px); background: rgba(249, 115, 22, 0.05); }
    .play-overlay { background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(2px); opacity: 0; transition: opacity 0.3s ease; }
    .video-card:hover .play-overlay { opacity: 1; }
    .qr-box { background: #fff; padding: 16px; border-radius: 2rem; display: inline-block; box-shadow: 0 0 40px rgba(249, 115, 22, 0.2); }
</style>";
    ...
    <div class='max-w-4xl mx-auto space-y-12'>
        ...
        <header class='flex justify-between items-center'>
            <div class='flex items-center gap-6'>
                <div class='w-20 h-20 bg-orange-500/10 rounded-[2rem] flex items-center justify-center border border-orange-500/20'>
                    <span class='material-symbols-outlined text-orange-400 text-5xl'>{$icone}</span>
                </div>
                <div>
                    <h1 class='text-4xl font-bold headline text-white'>{$titulo}</h1>
                    <p class='text-orange-500/60 font-bold uppercase tracking-widest text-[10px] mt-1'>{$subtitulo}</p>
                </div>
            </div>
        ...
        <section class='space-y-6 text-center'>
             <h2 class='text-xl font-bold text-white flex items-center justify-center gap-2 headline'>
                <span class='material-symbols-outlined text-orange-400'>bolt</span>
                Laboratório Cinético VR
            </h2>
            <div class='video-card rounded-[3rem] overflow-hidden group cursor-pointer max-w-2xl mx-auto' onclick='abrirVideo(\"https://www.youtube.com/embed/{$videoID}\")'>
                ...
                <div class='p-8'>
                    ...
                    <p class='text-slate-500 text-xs uppercase font-bold tracking-widest'>Simulação Física • VR 360° • {$videoTempo}</p>
                </div>
            </div>
        </section>

        <section class='glass p-12 rounded-[4rem] border-orange-500/10 text-center bg-orange-500/[0.01]'>
            <h2 class='text-2xl font-bold text-white headline mb-4'>Sincronizar Meta Quest</h2>
            <p class='text-slate-400 text-sm mb-8'>Aponte o headset para visualizar a dinâmica das partículas e forças gravitacionais.</p>
            <div class='qr-box'>
                <img src='https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=https://youtu.be/{$videoID}' class='w-40 h-40'>
            </div>
            <p class='mt-6 text-orange-500 font-mono text-[10px] tracking-widest uppercase text-opacity-50'>Protocolo: PHYS-UNIT-09</p>
        </section>
    </div>
</div>
