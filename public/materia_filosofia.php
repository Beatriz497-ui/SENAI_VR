...
<style>
    ...
    .video-card:hover { border-color: #8b5cf6; transform: translateY(-8px); background: rgba(139, 92, 246, 0.05); }
    ...
    <div class='max-w-4xl mx-auto space-y-12'>
        ...
        <header class='flex justify-between items-center'>
            <div class='flex items-center gap-6'>
                <div class='w-20 h-20 bg-violet-500/10 rounded-[2rem] flex items-center justify-center border border-violet-500/20 shadow-[0_0_20px_rgba(139,92,246,0.1)]'>
                    <span class='material-symbols-outlined text-violet-400 text-5xl'>{$icone}</span>
                </div>
                <div>
                    <h1 class='text-4xl font-bold headline text-white'>{$titulo}</h1>
                    <p class='text-violet-500/60 font-bold uppercase tracking-widest text-[10px] mt-1'>{$subtitulo}</p>
                </div>
            </div>
        ...
        <section class='space-y-6 text-center'>
             <h2 class='text-xl font-bold text-white flex items-center justify-center gap-2 headline'>
                <span class='material-symbols-outlined text-violet-400'>auto_stories</span>
                O Mundo das Ideias em VR
            </h2>
            <div class='video-card rounded-[3rem] overflow-hidden group cursor-pointer max-w-2xl mx-auto' onclick='abrirVideo(\"https://www.youtube.com/embed/{$videoID}\")'>
                <div class='relative h-64 bg-slate-900'>
                    ...
                    <div class='play-overlay absolute inset-0 flex items-center justify-center'>
                        <div class='w-20 h-20 bg-violet-500 rounded-full flex items-center justify-center shadow-lg'>
        ...
        <section class='glass p-12 rounded-[4rem] border-violet-500/10 text-center bg-violet-500/[0.01]'>
            <h2 class='text-2xl font-bold text-white headline mb-4'>Reflexão no Meta Quest</h2>
            <p class='text-slate-400 text-sm mb-8'>Escanear para vivenciar a Alegoria da Caverna e questionar a percepção da realidade através da imersão.</p>
            <div class='qr-box'>
                <img src='https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=https://youtu.be/{$videoID}' class='w-40 h-40'>
            </div>
            <p class='mt-6 text-violet-400 font-mono text-[10px] tracking-widest uppercase'>Protocolo: PHILO-SOPHIA-VR</p>
        </section>
    </div>
</div>
