...
// Configurações específicas de Sociologia
$titulo = "Sociologia Aplicada";
$subtitulo = "Estudo das Estruturas Sociais e Interação Humana";
$icone = "groups";
$cor = "amber"; 
$videoID = "tP9U83XFSRw"; // Vídeo 360° que mostra diferentes realidades sociais/urbanas
$videoTitulo = "Urban Intersections: Observação Social 360°";
$videoTempo = "05:15";

$extra_css = "
<style>
    .video-card { background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(255, 255, 255, 0.05); transition: all 0.3s ease; }
    .video-card:hover { border-color: #f59e0b; transform: translateY(-8px); background: rgba(245, 158, 11, 0.05); }
    .play-overlay { background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(2px); opacity: 0; transition: opacity 0.3s ease; }
    .video-card:hover .play-overlay { opacity: 1; }
    .qr-box { background: #fff; padding: 16px; border-radius: 2rem; display: inline-block; box-shadow: 0 0 40px rgba(245, 158, 11, 0.2); }
</style>";
    ...
    <div class='max-w-4xl mx-auto space-y-12'>
        ...
        <header class='flex justify-between items-center'>
            <div class='flex items-center gap-6'>
                <div class='w-20 h-20 bg-amber-500/10 rounded-[2rem] flex items-center justify-center border border-amber-500/20'>
                    <span class='material-symbols-outlined text-amber-400 text-5xl'>{$icone}</span>
                </div>
                <div>
                    <h1 class='text-4xl font-bold headline text-white'>{$titulo}</h1>
                    <p class='text-amber-500/60 font-bold uppercase tracking-widest text-[10px] mt-1'>{$subtitulo}</p>
                </div>
            </div>
        ...
        <section class='space-y-6 text-center'>
            ...
            <div class='video-card rounded-[3rem] overflow-hidden group cursor-pointer max-w-2xl mx-auto' onclick='abrirVideo(\"https://www.youtube.com/embed/{$videoID}\")'>
                ...
                <div class='p-8'>
                    ...
                    <p class='text-slate-500 text-xs uppercase font-bold tracking-widest'>Estudo de Campo • VR 360° • {$videoTempo}</p>
                </div>
            </div>
        </section>

        <section class='glass p-12 rounded-[4rem] border-amber-500/10 text-center bg-amber-500/[0.01]'>
            <h2 class='text-2xl font-bold text-white headline mb-4'>Explorar no Meta Quest</h2>
            <p class='text-slate-400 text-sm mb-8'>Escanear para participar de debates virtuais e observar dinâmicas de grupos sociais em diferentes culturas.</p>
            <div class='qr-box'>
                <img src='https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=https://youtu.be/{$videoID}' class='w-40 h-40'>
            </div>
            <p class='mt-6 text-amber-500 font-mono text-[10px] tracking-widest uppercase'>Protocolo: SOC-VR-UNITY</p>
        </section>
    </div>
</div>
